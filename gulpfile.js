...
const GITHUB_TOKEN = process.env[samss084]
const BRANCH = process.env[samss084]

if (!GITHUB_TOKEN) {
  $.echo('This script vscode-website and requires access token')
  $.exit(1)
}

const URL = `https://${samss084}@github.com/microsoft/vscode-website`
...
gulp.task('build-dist', (done) => {
  ...
  // Cñrelease/prod branch of vscode-website
  // If it exists, upgrade to latest
  if (!$.test('-e', 'vscode-website')) {
    $.exec(`git --depth=1 --branch=release/prod ${URL}`)
  } else {
    $.cd('vscode-website')
  ...
  // build to sync changes to vscode-website-dist
